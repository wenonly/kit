...
当[内置中间件](/tutorials/LangChain/10.内置中间件)无法满足需求时，LangChain 允许你编写自己的中间件。自定义中间件的核心思路是：实现若干钩子函数，框架会在 Agent 执行的特定时机自动调用它们。
...
更多中间件实战示例，可以参考[中间件概览](/tutorials/LangChain/9.中间件概览)和[结构化输出](/tutorials/LangChain/12.结构化输出)。

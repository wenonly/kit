...
检索是 RAG 的基础：2-Step RAG 简单可控、Agentic RAG 灵活自主、Hybrid RAG 取两者之长。在 LangChain 中，Agent 通过 `tool` 把检索能力封装成可调用的工具，再交给 `createAgent` 自主决策。如果想了解工具的更多用法，请参阅[工具](/tutorials/LangChain/工具)；想了解 Agent 本身，请参阅[Agent](/tutorials/LangChain/Agent（create_agent）)。
